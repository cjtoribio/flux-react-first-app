...
var errorCodes= require('../utils/errorCodes')['characters'];
...
module.exports = function(app) {
	...
	app.post('/api/characters', function(req, res, next) {
		...
		return;
		function getCharacter(next){ // (parsedXml)
				...
				parser.parseString(xml, function(err, parsedXml) {
					try {
						...
						models.Character.findOne({
							...
							if (err) return next(err);
							if (character) {
								return next(errorCodes.CHARACTER_EXISTS(null, character.name));
							}

							next(err, characterId);
						});
					} catch (e) {
						next( errorCodes.XML_PARSE_ERROR() );
					}
				});
			});
		}
		function getCharacterInfo(characterId, next){
			...
			request.get({
				...
				parser.parseString(xml, function(err, parsedXml) {
					...
					try {
						...
						next( errorCodes.CHARACTER_INVALID(e, characterName) );
					}
				});
			});
		}
	...
	app.get('/api/characters', function(req, res, next) {
		...
		models.Character.find({ random: { $near: [Math.random(), 0] } })
			.where('voted', false)
			...
			.exec(function(err, characters) {
				...
				models.Character
					...
					.exec(function(err, characters) {
						...
						models.Character.update({}, { $set: { voted: false } }, { multi: true }, function(err) {
							if (err) return next(err);
							res.send([]);
						});
					});
	...
	app.get('/api/characters/count', function(req, res, next) {
		models.Character.count({}, function(err, count) {
			if (err) return next(err);
